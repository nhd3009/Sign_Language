...
count = len(oss.listdir("D:\\demo app\\SignLanguage\\dataSet\\trainData\\A\\"))
    ...
    try:
        ...
        if interrupt & 0xFF == ord('n'):
            ...
            if ord(c_dir)==ord('Z')+1:
                c_dir='A'
            flag = False
            count = len(oss.listdir("dataSet\\trainData\\" + (c_dir) + "\\"))
        ...
        if flag==True:
            ...
            if suv==180:
                flag=False
            if step%3==0:
                cv2.imwrite("dataSet\\trainData\\" + (c_dir) + "\\" + str(count) + ".jpg",
                            skeleton1)
